...
void frame( // Process motion for a single frame of left or right eye
  ...
  // Keep the original movement generator, restarting smoothly from the
  // held position on release instead of jumping to a hidden random position.
  if (touchGazeHeld()) {
    eyeX = eyeOldX = eyeNewX = touchGazeX();
    eyeY = eyeOldY = eyeNewY = 512;
    eyeInMotion = false;
    eyeMoveDuration = 0;
    eyeMoveStartTime = t;
  }
  ...
  if (touchBlinkPending && eye[eyeIndex].blink.state == NOBLINK) {
    ...
    eye[eyeIndex].blink.duration = random(36000, 72000);
    touchBlinkPending = false;
  }
